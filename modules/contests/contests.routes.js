...
const { createContest, getContestsByCreator, deleteContest, updateContest, getContests, updateContestStatus, getApprovedContests } = require('./contest.controller');

router.get('/all', verifyFirebaseToken(usersCollection), requireRole('admin'), getContests)
router.get('/', verifyFirebaseToken(usersCollection), getApprovedContests)
